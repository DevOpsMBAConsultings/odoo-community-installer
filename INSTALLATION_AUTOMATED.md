...
Guía rápida para instalar **Odoo 19 Community** usando el script automatizado, incluyendo módulos OCA y módulos propios como Digifact.
...
- Servidor Ubuntu 24.04 LTS limpio
- Acceso root o sudo
...
| **¿Instalar módulos OCA para Odoo 19?** | `N` | Escribe **`s`** para instalar automáticamente los módulos OCA para Odoo 19 |
...
**`addons_path` en `/etc/odoo19.conf` (con OCA):**
```ini
...
grep addons_path /etc/odoo19.conf | tr ',' '\n' | xargs -I{} ls -d {} 2>&1
```
...
```bash
sudo -u odoo /opt/odoo/odoo19/venv/bin/python3 /opt/odoo/odoo19/odoo/odoo-bin   -c /etc/odoo19.conf   -d odoo19   -u nombre_del_modulo   --stop-after-init
sudo systemctl start odoo19
```
...
```bash
cd /path/to/odoo-community-installer-19
