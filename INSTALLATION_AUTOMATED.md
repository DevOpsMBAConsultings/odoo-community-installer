...
Guía rápida para instalar **Odoo 17 Community** usando el script automatizado, incluyendo módulos OCA y módulos propios como Digifact.
...
| **¿Instalar módulos OCA para Odoo 17?** | `N` | Escribe **`s`** para instalar automáticamente los módulos OCA para Odoo 17 |
...
**`addons_path` en `/etc/odoo17.conf` (con OCA):**
```ini
...
```bash
sudo -u odoo /opt/odoo/odoo17/venv/bin/python3 /opt/odoo/odoo17/odoo/odoo-bin   -c /etc/odoo17.conf   -d odoo17   -u nombre_del_modulo   --stop-after-init
sudo systemctl start odoo17
```
...
```bash
cd /path/to/odoo-community-installer-17
