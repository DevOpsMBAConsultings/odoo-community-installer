# Odoo 16 Community Install – Qué hace el script (Checklist)

Usa esta lista para ver todo lo que hace el script y verificar que nada falta tras una instalación limpia.

> **Versión: Odoo 16** (LTS — para clientes en versiones legacy).
...
| 04 | `04_clone_odoo.sh` | Clonar fuente de Odoo 16 en `/opt/odoo/odoo16/odoo` |
| 05 | `05_python_venv.sh` | Python venv en `/opt/odoo/odoo16/venv` |
| 06 | `06_python_dependencies.sh` | Instalar `requirements.txt` de Odoo + **wand** |
| 07 | `07_odoo_config.sh` | Generar `/etc/odoo16.conf` desde template (DB, admin password, addons_path con rutas OCA incluidas) |
| 07 | `07_systemd_service.sh` | Crear y habilitar el servicio systemd `odoo16` |
...
| **Rama** | `16.0`. Si no existe, usa la rama por defecto |
| **Destino** | `/opt/odoo/oca/<repo-name>/` — separado de los addons propios |
| **addons_path** | Cada `/opt/odoo/oca/<repo>` ya fue inyectado en `/etc/odoo16.conf` por el paso 07 vía `{{OCA_ADDON_PATHS}}` |
...
**addons_path resultante en `/etc/odoo16.conf` (con OCA):**
```ini
...
**Repos OCA disponibles para Odoo 16** (definidos en `config/oca_repos.conf`):
...
|----------|-----------|-----------  |
| `DB_NAME` | `odoo${ODOO_VERSION}` | e.g. `odoo16` |
...
|---|---|
| Servicio Odoo activo | `sudo systemctl status odoo16` |
...
| addons_path incluye OCA | `grep addons_path /etc/odoo16.conf` debe mostrar rutas `/opt/odoo/oca/*` |
| OCA repos clonados | `ls /opt/odoo/oca/` debe listar todos los repos seleccionados |
| Rutas no fantasma | `grep addons_path /etc/odoo16.conf | tr ',' '\n' | xargs -I{} ls -d {} 2>&1` — ninguna línea con "No such file" |
...
| Actualizar repos OCA | `sudo -u odoo git -C /opt/odoo/oca/<repo> pull && systemctl restart odoo16` |
...
*Documentación actualizada para reflejar el soporte de módulos OCA con selección automática por versión en Odoo 16.*
