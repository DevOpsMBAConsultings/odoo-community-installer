...
|------|-----------|-------|
| Ubuntu 22.04 LTS (o Ubuntu 24.04 LTS) | ✅ | VM limpia o bare metal |
...
| 04 | `04_clone_odoo.sh` | Clonar fuente de Odoo 17 en `/opt/odoo/odoo17/odoo` |
| 05 | `05_python_venv.sh` | Python venv en `/opt/odoo/odoo17/venv` |
| 06 | `06_python_dependencies.sh` | Instalar `requirements.txt` de Odoo + **wand** |
| 07 | `07_odoo_config.sh` | Generar `/etc/odoo17.conf` desde template (DB, admin password, addons_path con rutas OCA incluidas) |
| 07 | `07_systemd_service.sh` | Crear y habilitar el servicio systemd `odoo17` |
...
| **Rama** | `17.0`. Si no existe, usa la rama por defecto |
| **Destino** | `/opt/odoo/oca/<repo-name>/` — separado de los addons propios |
| **addons_path** | Cada `/opt/odoo/oca/<repo>` ya fue inyectado en `/etc/odoo17.conf` por el paso 07 vía `{{OCA_ADDON_PATHS}}` |
...
**addons_path resultante en `/etc/odoo17.conf` (con OCA):**
```ini
addons_path = /opt/odoo/auto-addons,/opt/odoo/odoo17/odoo/addons,/opt/odoo/oca/account-financial-reporting,/opt/odoo/oca/account-financial-tools,...,/opt/odoo/custom-addons
```

**Repos OCA disponibles para Odoo 17** (definidos en `config/oca_repos.conf`):
...
|----------|-----------|-----------  |
| `DB_NAME` | `odoo${ODOO_VERSION}` | e.g. `odoo17` |
...
|---|---|
| Servicio Odoo activo | `sudo systemctl status odoo17` |
...
| addons_path incluye OCA | `grep addons_path /etc/odoo17.conf` debe mostrar rutas `/opt/odoo/oca/*` |
| OCA repos clonados | `ls /opt/odoo/oca/` debe listar todos los repos seleccionados |
| Rutas no fantasma | `grep addons_path /etc/odoo17.conf | tr ',' '\n' | xargs -I{} ls -d {} 2>&1` — ninguna línea con "No such file" |
...
| Actualizar repos OCA | `sudo -u odoo git -C /opt/odoo/oca/<repo> pull && systemctl restart odoo17` |
...
*Documentación actualizada para reflejar el soporte de módulos OCA con selección automática por versión en Odoo 17.*
