...
| 04 | `04_clone_odoo.sh` | Clonar fuente de Odoo 18 en `/opt/odoo/odoo18/odoo` |
...
| **Rama** | `18.0`. Si no existe, usa la rama por defecto |
| **Destino** | `/opt/odoo/oca/<repo-name>/` — separado de los addons propios |
| **addons_path** | Cada `/opt/odoo/oca/<repo>` ya fue inyectado en `/etc/odoo18.conf` por el paso 07 vía `{{OCA_ADDON_PATHS}}` |
...
*Documentación actualizada para reflejar el soporte de módulos OCA con selección automática por versión en Odoo 18.*
