# MBA – Odoo 17 Community Installer

Proceso de instalación estandarizado y repetible para **Odoo 17 Community** en **Ubuntu 22.04 LTS (o Ubuntu 24.04 LTS)** (producción estable).

## 🎯 Objetivo
- Instalar Odoo 17 Community de forma limpia, controlada y automatizada.
...
- **Entorno aislado exclusivo para Odoo 17**: usando Python 3.10 (se instala automáticamente en Ubuntu 24.04 vía deadsnakes PPA o viene nativo en 22.04) y un entorno virtual (`venv`) PEP 668 safe.
- Configuración de Odoo generada desde plantillas.
- Creación y habilitación del servicio systemd (`odoo17.service`).
- **Instalación automática de módulos OCA para Odoo 17**:
  - Lee los repositorios OCA compatibles en `config/oca_repos.conf`.
  - Clona cada repo en `/opt/odoo/oca/` (rama `17.0`).
  - Agrega automáticamente cada ruta al `addons_path` en `/etc/odoo17.conf`.
...
- **Python**: Python 3.10 (se instala automáticamente en Ubuntu 24.04 vía deadsnakes PPA o viene nativo en 22.04)
...
```
/opt/odoo/
├── odoo17/                 # Código fuente core de Odoo 17
...
└── oca/                    # Repositorios oficiales de la OCA (rama 17.0)
...
## 📦 Módulos OCA para Odoo 17

El instalador detecta los módulos OCA configurados para Odoo 17 en `config/oca_repos.conf` y te ofrece instalarlos automáticamente.

1. Clona cada repo configurado en `/opt/odoo/oca/<repo-name>/` (rama `17.0`).
2. Agrega cada ruta al `addons_path` en `/etc/odoo17.conf`.
...
2. **Clona el repositorio e ingresa a él:**
    ...
    git clone -b 17.0 https://github.com/DevOpsMBAConsultings/odoo-community-installer.git odoo-community-installer-17
    cd odoo-community-installer-17
    ```
