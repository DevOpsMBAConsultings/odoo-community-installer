# MBA – Odoo 16 Community Installer

Proceso de instalación estandarizado y repetible para **Odoo 16 Community** en **Ubuntu 22.04 LTS (o Ubuntu 24.04 LTS)** (LTS — para clientes en versiones legacy).

## 🎯 Objetivo
- Instalar Odoo 16 Community de forma limpia, controlada y automatizada.
...
- **Entorno aislado exclusivo para Odoo 16**: usando Python 3.10 (se instala automáticamente en Ubuntu 24.04 vía deadsnakes PPA o viene nativo en 22.04) y un entorno virtual (`venv`) PEP 668 safe.
- Configuración de Odoo generada desde plantillas.
- Creación y habilitación del servicio systemd (`odoo16.service`).
- **Instalación automática de módulos OCA para Odoo 16**:
  - Lee los repositorios OCA compatibles en `config/oca_repos.conf`.
  - Clona cada repo en `/opt/odoo/oca/` (rama `16.0`).
  - Agrega automáticamente cada ruta al `addons_path` en `/etc/odoo16.conf`.
...
└── oca/                    # Repositorios oficiales de la OCA (rama 16.0)
...
## 📦 Módulos OCA para Odoo 16

El instalador detecta los módulos OCA configurados para Odoo 16 en `config/oca_repos.conf` y te ofrece instalarlos automáticamente.

1. Clona cada repo configurado en `/opt/odoo/oca/<repo-name>/` (rama `16.0`).
2. Agrega cada ruta al `addons_path` en `/etc/odoo16.conf`.
